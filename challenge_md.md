...
Any machine can point out PEP8 errors, and by the time I reach code review the program tends to run without crashing.  Thus, the most important thing a reviewer can do for my code is to make it more pythonic.  A good reviewer will find confusing names and make them less confusing (I've seen an anacondas environments named 'pipenv', and Docker images named 'python').  A good reviewer will notice when I'm using a verbose for loop instead of a list comprehension, and when there is a simple alternative formulation that would make the code run 10x faster.  A good reviewer will note which elements of the program are good units for testing, and think of edge cases that could break the code as it is written.  A good reviewer is a subclass of good editor: they will maximize the code's simplicity, increase its elegance, and help it reach towards grace.

- We have an awful lot of computers here, and it gets pretty confusing with
  slightly different things running on all of them. How could containers help us
  improve this situation?

Containers help us keep applications isolated in abstract boxes (i.e., containers) that already contain all the third-party programs they need in order to function (e.g., pandas, matplotlib) in the correct versions the apps were built to work with. Containers use the host operating system, so in exchange for not being quite as isolated as virtual machines they are significantly lighter.  Containers can be defined by a simple file that spells out all the programs that go into the container.  That file can be published, and any user can use those instructions to build the containers on a new computer.  Containers help keep apps isolated from the host computer, and provide everything they need to work exactly as they were designed.  This modularity ensures that the apps always work as advertised, and that their effects are contained in case something goes wrong with them.
